...
def load_samples() -> dict[str, list[str]]:
    """Load sample texts from sample-text directory"""
    samples = {"vi": [], "en": []}
    sample_dir = Path("sample-text")
    
    for lang in ["vi", "en"]:
        lang_dir = sample_dir / lang
        if lang_dir.exists():
            for txt_file in sorted(lang_dir.glob("*.txt")):
                try:
                    content = txt_file.read_text(encoding="utf-8").strip()
                    if content:
                        samples[lang].append(content)
...
print(f"📝 Loaded samples: VI={len(SAMPLES['vi'])}, EN={len(SAMPLES['en'])}")
...
def get_random_sample(lang: str) -> str:
    """Get a random sample text for given language"""
...
with gr.Blocks(
    ...
    with gr.Row(equal_height=True, elem_id="main-row"):
        ...
        with gr.Column(scale=1, elem_classes=["card"]):
            ...
            # Pre-fill with random Vietnamese sample
            initial_sample = get_random_sample("vi")
